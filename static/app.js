...
const buildUrl = (path, queryObj = {}) => {
    const entries = Object.entries(queryObj).filter(([, value]) => value !== undefined && value !== null);
...
async function fetchAndRenderContent(tabId) {
    ...
    try {
        ...
        if (url) {
            ...
            if (tabId === 'webtoon' || tabId === 'novel') {
                 ...
                 if (day !== 'completed' && day !== 'hiatus' && day !== 'all') {
                     data = (json && Array.isArray(json[day]) ? json[day] : []) || [];
                 } else if (day === 'all') {
                     // Aggregate all days
        ...
        const normalizeList = (list) => Array.isArray(list)
            ? list.map((item) => ({ ...item, meta: normalizeMeta(item?.meta) }))
            : [];
...
function createCard(content, tabId, aspectClass) {
    ...
    const thumb = meta?.common?.thumbnail_url || 'https://via.placeholder.com/300x400/333/999?text=No+Img';
    ...
    <div class="px-0.5">
        ...
        <p class="text-[11px] text-[#A3A3A3] mt-0.5 truncate">${meta?.common?.authors?.join(', ') || ''}</p>
    </div>
`;
