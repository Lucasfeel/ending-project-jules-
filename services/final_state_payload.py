...
def _get_override_field(override, key):
    if override is None:
        return None
    if isinstance(override, dict):
        return override.get(key)
    # Support row-like objects
    try:
        return override[key]
    except Exception:
        return None
...
def build_final_state_payload(content_status, override=None, now=None):
    ...
    override_status = _get_override_field(override, "override_status")
    override_completed_at = _get_override_field(override, "override_completed_at")
