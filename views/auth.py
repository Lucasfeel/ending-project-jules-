...
def register():
    ...
    if not email or not password:
        return _error_response(400, 'INVALID_REQUEST', '이메일과 비밀번호가 필요합니다.')
    if not is_valid_email(email):
        return _error_response(400, 'INVALID_REQUEST', '올바른 이메일 형식이 아닙니다.')

    try:
        user, error = register_user(email, password)
        if error:
            return _error_response(400, 'DUPLICATE_EMAIL', error)
        return jsonify({'success': True, 'user_id': user['id']}), 201
    except psycopg2.Error:
        return _error_response(500, 'DATABASE_ERROR', '데이터베이스 오류가 발생했습니다.')
    except Exception:
        return _error_response(500, 'SERVER_ERROR', '서버 오류가 발생했습니다.')
